...
"""
    ...
    Paramètres : id_user_sel = 0 >> tous les films.
                 id_user_sel = "n" affiche le film dont l'id est "n"
...
"""
    nom: ediutilisateur_film_selected
...
@app.route("/ediutilisateur_film_selected", methods=['GET', 'POST'])
def ediutilisateur_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
            ...
            # href="{{ url_for('ediutilisateur_film_selected', id_film_genres_edit_html=row.id_film) }}"
        ...
        except Exception as Exception_ediutilisateur_film_selected:
            raise ExceptionEditGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                 f"{ediutilisateur_film_selected.__name__} ; "
                                                 f"{Exception_ediutilisateur_film_selected}")
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            new_lst_inutilisateur_film_old = list(map(int, new_lst_str_genres_films))
            print("new_lsutilisateur_film ", new_lst_inutilisateur_film_old, "type new_lsutilisateur_film ",
                  type(new_lst_inutilisateur_film_old))
            ...
            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "utilisateur_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_user_ins in lst_diff_genres_insert_a:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_user_ins" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_film": id_film_selected,
                                                               "value_fk_genre": id_user_ins}

                    mconn_bd.execute(strsql_inserutilisateur_film, valeurs_film_sel_genre_sel_dictionnaire)

                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "utilisateur_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_user_del in lst_diff_genres_delete_b:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_user_del" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_film": id_film_selected,
                                                               "value_fk_genre": id_user_del}
    ...
    # Après cette mise à jour de la table intermédiaire "utilisateur_film",
